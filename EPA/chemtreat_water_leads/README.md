...
| `rule_treatable_permit_parameter` | 15 | **Pre-violation signal.** +5 per ChemTreat-treatable parameter class on the facility's NPDES permit (phosphorus, ammonia, TSS, BOD, oil/grease, metals — incl. Fe/Mn, cyanide, chlorine residual), capped. Reads `permit_has_*` columns populated by `bulk_loader.stream_permit_limits` from `npdes_limits.zip`. Bulk-only signal; pipeline.run leaves these columns empty and the rule cleanly returns None there. |
