...
| **WET** | Whole Effluent Toxicity | A pass/fail biological assay required on some NPDES permits — expose live organisms to the discharge, see if they survive. Reported as a binary in EPA's data, so when our DMR streamer sees a WET failure it shows up as the INT32_MAX "infinite over zero" sentinel; the severe tier (+15) still fires and the score reason now reads "pass/fail or limit-of-zero parameter failure" instead of a misleading percentage. |
| **BOD** | Biochemical Oxygen Demand | A core CWA parameter: how much dissolved oxygen the discharge consumes. High BOD chokes downstream waters. One of ChemTreat's treatable parameter classes (`permit_has_bod`); when it appears in `score_reasons` it means the facility's permit covers BOD or they're currently exceeding it. |
| **TSS** | Total Suspended Solids | A core CWA parameter: weight of solid particles in the discharge. Also in ChemTreat's treatable bucket (`permit_has_tss`). |
...
| **POTW** | Publicly Owned Treatment Works | The CWA-side counterpart to PWS — a municipal **wastewater** treatment plant. The CSO/SSO integration targets this segment specifically (NAICS prefix `22132` in `TARGET_NAICS`). Most sewer-overflow events the daily-cadence feed reports come from POTWs whose collection system or treatment train failed. |
