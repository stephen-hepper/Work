...
```
Score 142:
  +40: Significant Non-Complier (SNC text)
...
| `rule_treatable_permit_parameter` | 15 | **Pre-violation signal.** +5 per ChemTreat-treatable parameter class on the facility's NPDES permit (phosphorus, ammonia, TSS, BOD, oil/grease, metals — incl. Fe/Mn, cyanide, chlorine residual, microbiological — coliform / E. coli / Enterococci / fecal indicators), capped. Reads `permit_has_*` columns populated by `bulk_loader.stream_permit_limits` from `npdes_limits.zip`. Bulk-only signal; pipeline.run leaves these columns empty and the rule cleanly returns None there. |
...
└── markdown/                  # All package docs live here
    ...
    ├── EXTERNAL_DATA_STATUS.md  # Running status of external-data integrations
...
└── markdown/RATIONALE.md  # Viewer design notes & gap list

../chemtreat_sales_briefings/    # LLM-drafted regional briefings — downstream
├── __init__.py                  # consumer of snapshot.sqlite (read-only).
├── briefings.py                 # OpenAI/Azure tool-use loop + email send + CLI
├── state.py                     # briefings_state.sqlite — which leads briefed
├── regions.py                   #   when, plus the candidate gating predicate
├── naics.py                     # NAICS code → human industry name lookup
├── README.md                    # Briefings architecture + run instructions
└── .env.example                 # Template for OPENAI_API_KEY / SMTP / Azure
```
