...
| 3 | NPDES DMR archive (`npdes_dmrs_fy<YEAR>.zip`, ~344 MB compressed / 5 GB unzipped, weekly current-FY refresh) | `shipped` | **Active-compliance signal** — answers "which chemicals is the facility currently exceeding, by how much?". `rule_recent_dmr_exceedance` (tiered 5/8/10/12/15 by severity), `rule_exceeds_treatable_parameter` (+15 composite "permit covers it AND they're exceeding it"), `tag_recent_exceedance`, `tag_exceeds_treatable_parameter`, and four new columns (`recent_dmr_exceedances_count`, `top_exceeded_parameter`, `top_exceedance_pct`, `exceeded_treatable_parameters_text`). Closes the long-standing per-DMR depth gap (parameter / limit / dmr / exceedance %) nationwide — viewer's violation_events.csv now carries full per-DMR detail without requiring the API fine-comb. EPA column is **`EXCEEDENCE_PCT`** (typo, pinned in code); INT32_MAX sentinel for limit=0 rows clamped at 99,999% for display. |
| 4 | Sewer Overflow / CSO / SSO events (`current_sewer_overflow_and_collection_systems_tables.zip`, ~1 MB, daily; `ALL_CSO_downloads.zip`, ~300 KB, weekly) | `shipped` | **Active-compliance signal** — POTW lead segment specifically. The only EPA water dataset with daily cadence; closes the 30–90d reporting lag for sewer overflow / bypass events. Two complementary feeds: the eRule Phase 2 events archive (events + types + collection_system_permits) plus the National CSO Inventory (CSS membership for permits the eRule hasn't onboarded yet). `rule_recent_sewer_overflow` (tier ladder on type × wet/dry × volume, 5/8/12/15), `rule_combined_sewer_system` (+5 flat), `rule_collection_system_population` (mirrors SDWA tiers, 4/7/10 at 3K/10K/50K). Four new tags (`tag_recent_sewer_overflow`, `tag_recent_sso`, `tag_dry_weather_overflow`, `tag_combined_sewer_system`) plus 12 new columns (`recent_sewer_overflow_*`, `has_dry_weather_overflow`, `percent_collection_system_css`, `collection_system_population`, `has_combined_sewer_system`). Shipped alongside widening `TARGET_NAICS` to include `22132` (Sewage Treatment Facilities) so POTW leads actually enter the inventory. Live 2026-06-16 nationwide run: 67 CWA leads tagged with active sewer-overflow signal (0.19%, matches the eRule state-rollout sparseness), 622 leads with collection-system enrollment, 164 supplemented from the CSO Inventory feed; ~8.5K new POTW facilities entered the inventory. Full schema/wiring/risk discussion in `CSO_SSO_PLAN.md`. |
